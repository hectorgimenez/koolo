...
// 	const maxFindItemAttempts = 1
// 	const maxRange = 15
// 	const findItemWaitTime = time.Millisecond * 100
...
// 			if findItemAttempts < maxFindItemAttempts {
// 				foundCorpse := s.FindItemOnNearbyCorpses(d, maxRange, findItemWaitTime)
// 				if foundCorpse {
// 					findItemAttempts++
// 					steps = append(steps, step.Wait(findItemWaitTime))
// 					emptyIterations = 0
// 				} else {
// 					emptyIterations++
// 				}
...
// 				s.logger.Debug("No monsters or corpses found for several iterations, exiting sequence")
// 				return nil
// 			}
// 			steps = append(steps, step.Wait(time.Millisecond*100))
// 		}
...
func (s Berserker) FindItemOnNearbyCorpses(d game.Data, maxRange int, waitTime time.Duration) bool {
	...
	for _, corpse := range d.Corpses {
		...
		if (corpse.Type != data.MonsterTypeChampion &&
			...
			corpse.Type != data.MonsterTypeSuperUnique) || distance > maxRange {
			s.logger.Debug("Skipping corpse: not a desired monster type or too far",
		...
		// // Directly check if the cursor is over any unit
		...
		// 	}
		// } else {
		// 	s.logger.Debug("Cursor not over any unit, skipping", slog.Any("corpse_id", corpse.UnitID))
		// }
